...
# --- CONFIGURE ---
FROM_EMAIL = "no-reply@acountprotetion-micoscoft.com"
FROM_NAME  = "TEST CORPORATIONS LTD"
TO_EMAIL   = "blazerdelivery@gmail.com"
SUBJECT    = "Security Notice — Action Recommended"
ATTACH_PATH = "invoice.pdf"   # set to a real file or leave as-is to skip attachment
# ------------------

plain_text = (
    "Hello,\n\n"
    "This is a security notice from TEST CORPORATIONS LTD. "
    "If you did not perform recent account activity, please contact support.\n\n"
    "Thanks,\nTEST CORPORATIONS LTD"
)
...
def build_message():
    ...
    m.add_content(Content("text/plain", plain_text))
    m.add_content(Content("text/html", html_body))
...
def send():
    ...
    if not api_key:
        raise SystemExit("SENDGRID_API_KEY not set in environment (export SENDGRID_API_KEY='SG...')")

    message = build_message()

    try:
        sg = SendGridAPIClient(api_key)
        # sg.set_sendgrid_data_residency("eu")   # uncomment only if you use EU data residency
